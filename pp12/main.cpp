...
void makeCheckImage(void)
{
    ...
    for (i = 0; i < checkImageHeight; i++) { //바이튼 단위로 올림 색상을 정해준다 체크문의 그리는거
        for (j = 0; j < checkImageWidth; j++) {
            c = ((((i & 0x8) == 0) ^ ((j & 0x8)) == 0)) * 255;//0x8 = 8 16진수에 정중앙 (^ 두개다 같으면 0 한개라도 다르면 0)
            checkImage[i][j][0] = (GLubyte)c;//RGB
            checkImage[i][j][1] = (GLubyte)c;
            checkImage[i][j][2] = (GLubyte)c;
            checkImage[i][j][3] = (GLubyte)255;//GLubyte 오픈지엘에서 음수없이 양수만 있음
        }
    }
}
//싱볼릭 링크 바로가기
void init(void)
{
    ...
    makeCheckImage();
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);//패킹을 안한다.

    glGenTextures(1, &texName);//텍스터 설정한다. 바인딩 해준다. 바인딩 묵는거 아래 둘을 묶어줌 1번에 그림을 만들어주고 이름을 textname이다 2d이미지다.
    glBindTexture(GL_TEXTURE_2D, texName);//앞에가 주소갑 이 주소갑에 있는 놈을 
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
        GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, checkImageWidth,//RGB다 쓴다.
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);//GL_DECAL 텍스트를 입겠다.
        glBindTexture(GL_TEXTURE_2D, texName);
        glBegin(GL_QUADS);//점4개 찍으면 네모 단들어짐
